...
set "SRC=%ROOT%vendor\whisper.cpp"
set "BUILD=%ROOT%build\whisper.cpp"
...
if not exist "%SRC%\CMakeLists.txt" (
  echo No se encontro whisper.cpp en tools\vendor
  exit /b 1
)
...
"%CMAKE%" --build "%BUILD%" --config Release --target whisper-cli whisper-server -j 4
if errorlevel 1 exit /b 1

echo whisper.cpp listo en %BUILD%
